...
from metagrad.utils import clip_grad_norm_
...
class EncoderRNN(nn.Module):
    ...
    def initHidden(self):
        return Tensor.zeros((1, 1, self.hidden_size), device=device)
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden):
        ...
        output, hidden = self.gru(output, hidden)
        output = F.log_softmax(self.out(output[0]), axis=1)
        return output, hidden
...
class AttnDecoderRNN(nn.Module):
    ...
    def forward(self, input, hidden, encoder_outputs):
        ...
        attn_weights = F.softmax(
            self.attn(F.cat((embedded[0], hidden[0]), 1)), axis=1)

        attn_applied = F.bmm(attn_weights.unsqueeze(0), encoder_outputs.unsqueeze(0))

        output = F.cat((embedded[0], attn_applied[0]), axis=1)
...
def tensorFromSentence(lang, sentence):
    ...
    return Tensor(indexes, dtype=np.int32, device=device).view(-1, 1)
...
def train(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion,
    ...
    encoder_outputs = Tensor.zeros((max_length, encoder.hidden_size), device=device)
...
def evaluate(encoder, decoder, sentence, max_length=MAX_LENGTH):
    with no_grad():
        ...
        decoder_input = Tensor([[SOS_token]], device=device)  # SOS

        decoder_hidden = encoder_hidden

        decoded_words = []
        decoder_attentions = Tensor.zeros((max_length, max_length))

        for di in range(max_length):
            decoder_output, decoder_hidden, decoder_attention = decoder(
                decoder_input, decoder_hidden, encoder_outputs)
            decoder_attentions[di] = decoder_attention
            topi = decoder_output.data.argmax(axis=1)
